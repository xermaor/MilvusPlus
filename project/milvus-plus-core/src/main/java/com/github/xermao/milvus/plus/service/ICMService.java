...
import com.github.xermao.milvus.plus.converter.MilvusConverter;
...
public interface ICMService {
    ...
    default void createCollection(MilvusEntity milvusEntity) {
        ...
        try {
            String collectionName = milvusEntity.collectionName();
            // 检查集合是否存在
    ...
    default void addField(String collectionName, AddFieldReq... addFieldReq) {
        ...
        schemaBuilder.addField(List.of(addFieldReq));
    }
     ...
     * @return 字段架构信息
     */
    default CreateCollectionReq.FieldSchema getField(FieldFunction<String, ?> collectionName, FieldFunction<String, ?> fieldName) {
    ...
    default DescribeIndexResp describeIndex(String collectionName, FieldFunction<String, ?> fieldName) throws MilvusException {
    ...
    default void createAlias(MilvusEntity milvusEntity) {
        MilvusClientV2 client = getClient();
        for (String alias : milvusEntity.alias()) {
            CreateAliasReq createAliasReq = CreateAliasReq.builder().alias(alias)
                    .collectionName(milvusEntity.collectionName()).build();
    ...
    default ListAliasResp listAliases(MilvusEntity milvusEntity) {
        MilvusClientV2 client = getClient();
        return client.listAliases(ListAliasesReq.builder().collectionName(milvusEntity.collectionName()).build());
    }
}
